...
export default function generateSlip(formData, paymentData) {
  return new Promise((resolve, reject) => {
    try {
      ...
      try { doc.image(logoPath, 40, 15, { width: 60, height: 60 }); } 
      catch (e) { console.warn("Logo not found:", e.message); }
      ...
      if (formData.passport) {
        ...
        } catch (e) { console.warn("Invalid passport base64:", e.message); }
      }
      ...
      } catch (e) { console.warn("Watermark logo not found:", e.message); }

      doc.moveDown(2);

      // STUDENT DETAILS (all fields)
      doc.fontSize(12)
